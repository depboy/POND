...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, int algo)
{
    ...
    {
        case ALGO_NEOSCRYPT:
            pblock->nVersion |= BLOCK_VERSION_NEOSCRYPT;
            break;
        case ALGO_GROESTL:
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (auxpow != NULL) {
        ...
        // print to log
        LogPrintf("PONDMiner: AUX proof-of-work found; our hash: %s ; parent hash: %s ; target: %s\n",
               hash.GetHex().c_str(),
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PONDMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PONDMiner : ProcessBlock, block not accepted");
    }
...
void static ThreadBitcoinMiner(CWallet *pwallet)
{
    ...
    try 
    {
        ...
        {
            case ALGO_NEOSCRYPT:
                GenericMiner(pwallet, ALGO_NEOSCRYPT);
                break;
            case ALGO_GROESTL:
            ...
            case ALGO_QUBIT:
				...
				break;
			case ALGO_LYRA2RE:
                GenericMiner(pwallet, ALGO_LYRA2RE);
                break;
        }
    }
    catch (boost::thread_interrupted)
    {
        LogPrintf("POND miner terminated\n");
        throw;
    }
}
