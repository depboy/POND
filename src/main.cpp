...
#if defined(NDEBUG)
# error "POND cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "POND Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight < 5040) // starting block reward of 2 POND
        nSubsidy = 2 * COIN ;
    else if (nHeight < 630000) // reward ramp-up during initial mining phase (78750000 total POND)
        nSubsidy = (nHeight + 1)/5040 * 2 * COIN;
    else if (nHeight < 1078560) // reward reduced by 3% compound every 2 weeks
        nSubsidy = 250 * pow(double(0.97044562), (nHeight + 1)/10080) * COIN; 
    else nSubsidy = 10 * COIN; // after block 1,708,560 block reward is fixed at 10 POND (~1.5% initial annual rate of inflation)
	
	return nSubsidy + nFees;
}

static const int64_t nTargetTimespan = 600; // 10 minutes (NUM_ALGOS * 120 seconds)
static const int64_t nTargetSpacing = 600; // 10 minutes (NUM_ALGOS * 120 seconds)
...
void CheckForkWarningConditions()
{
    ...
    // If our best fork is no longer within 144 blocks (+/- 6 hours if no one mines it)
    // of our head, drop it
